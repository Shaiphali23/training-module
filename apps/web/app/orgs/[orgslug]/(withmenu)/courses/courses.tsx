...
function Courses(props: CourseProps) {
  ...
  const usergroupsAvailable =
    currentPlan === 'personal' || currentPlan === 'family'
  ...
  const [usergroupResourceUuids, setUsergroupResourceUuids] =
  ...
  useEffect(() => {
    ...
    getUserGroups(org?.id, access_token)
      .then((res: any) => {
        ...
        if (
          selectedUsergroupId &&
          !list.some((ug: any) => String(ug.id) === selectedUsergroupId)
        ) {
  ...
  const filteredCourses = useMemo(() => {
    ...
    if (usergroupResourceUuids) {
      courses = courses.filter((course: any) =>
        usergroupResourceUuids.has(course.course_uuid)
      )
    }
    ...
    if (searchQuery.trim()) {
      courses = courses.filter((course: any) =>
        searchMatchesAny(
          [course.name, course.description, course.tags],
          searchQuery
        )
      )
    }
                ...
                {usergroupsAvailable && usergroups.length > 0 && (
                  <div className="relative flex items-center gap-1.5">
                    <div className="relative">
                      ...
                      >
                        <option value="">
                          {t('courses.usergroup_filter.all_courses')}
                        </option>
                    ...
                    {showUsergroupInfo && (
                      <div className="absolute top-full left-0 mt-2 z-50 w-72 bg-white nice-shadow rounded-lg p-3 border border-gray-100">
                        <p className="text-xs font-semibold text-gray-700 mb-1">
                          {t('courses.usergroup_filter.info_title')}
                        </p>
                        <p className="text-xs text-gray-500 leading-relaxed">
                          {t('courses.usergroup_filter.info_description')}
                        </p>
                      </div>
                    )}
                  </div>
